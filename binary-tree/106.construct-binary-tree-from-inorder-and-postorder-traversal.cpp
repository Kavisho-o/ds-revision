...
public:
    ...
    TreeNode* help(vi &postorder, umii &mp, int s, int e, int &idx){
        ...
        int mid=mp[rootVal];
        root->right=help(postorder,mp,mid+1,e,idx);
        root->left=help(postorder,mp,s,mid-1,idx);
    ...
    TreeNode* buildTree(vi& inorder, vi& postorder) {
        ...
        int idx=0;
        return help(postorder,mp,0,n-1,n-1);
